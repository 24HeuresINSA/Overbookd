...
describe('test peronal account and money transfer', ()=> {
    it('should make a money transfer', function () {
        ...
        cy.contains('user.log').click();
        cy.get('#input-142').type('5');
        cy.get('#input-146').type('virement test')
        cy.get('#app > div.v-dialog__content.v-dialog__content--active > div > div > div.v-card__actions > button > span');
        // submitted transfer
    });
})