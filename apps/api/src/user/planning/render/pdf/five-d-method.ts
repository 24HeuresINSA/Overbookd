...
export class FiveDMethod {
  static generateWorkflow(): Content[] {
    ...
    const methodTexts = [
      ...
      },
      {
        text: [
          ...
          " : dès que l’incident est terminé (et même si l’harceleur·se est toujours présent), faire le point calmement avec la personne harcelée pour la rassurer (“Puis-je m’asseoir à côté de toi ? Veux-tu que je demande de l’aide ? Veux-tu d’ici ?”). Indiquer que l’attitude de l’agressseur·se est interdite par la loi, qu’iel n’avait pas à faire ça, que c’est uniquement de sa faute et pas celle de la victime; agir comme un·e ami·e, sans être intrusif. Éviter de parler avec l’agresseur·se car la création d’un débat peut entraîner une possible violence.",
        ],
      },
    ];
    const sourceText = ["Source : handsaway.fr/temoin-harcelement-agir"];
    ...
    const source = this.generateSource(sourceText);
