...
Each top-level folder corresponds to a **layer** with strict import rules. The split is deliberate and load-bearing — see [`docs/02-architecture/dependency-hierarchy.md`](../02-architecture/dependency-hierarchy.md) for the deep dive.
...
The pattern: a controller validates the DTO, hands a domain object to a use case from `@overbookd/<domain>`, lets the domain return a result (or throw a domain error), and serializes the response. See [`docs/02-architecture/api-anatomy.md`](../02-architecture/api-anatomy.md) for the full breakdown.
...
A page calls a composable, which calls a store, which calls a repository, which talks to the API. See [`docs/02-architecture/web-anatomy.md`](../02-architecture/web-anatomy.md) for the full breakdown.
...
- [`docs/02-architecture/dependency-hierarchy.md`](../02-architecture/dependency-hierarchy.md)
