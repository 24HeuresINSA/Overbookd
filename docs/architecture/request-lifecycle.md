...
Traefik labels live on each service in the compose file. TLS termination happens here using the local CA cert (see [`docs/start-here/02-local-setup.md` → Trust the certificate](../start-here/02-local-setup.md#trust-the-certificate)).
...
|---|---|
| Browser shows a CORS or TLS error | Traefik routing / cert trust ([`02-local-setup.md`](../start-here/02-local-setup.md)) |
...
- [`docs/architecture/web-anatomy.md`](./web-anatomy.md)
- [`docs/operations/local-dev-gotchas.md`](../operations/local-dev-gotchas.md)
