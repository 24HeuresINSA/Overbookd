...
def getAllfa():
    return requests.get(f"https://{DOMAIN}/api/fa", headers={"Authorization": f"Bearer {TOKEN}"}, timeout=5).json()
...
def buildFaGearRequest(fa):
    faId = fa["id"]
    gearRequests = requests.get(f"https://{DOMAIN}/api/fa/{faId}/gear-requests", headers={
...
def updateFaGearRequest(faGearRequests):
    ...
    for index, gearRequest in enumerate(faGearRequests["requests"]):
        ...
        gearRequestData = {"gearId": gearId,
                           "quantity": gearRequest["quantity"]}
        if index == 0:
            newPeriod = {"start": gearRequest["rentalPeriod"]
                         ["start"], "end": gearRequest["rentalPeriod"]["end"]}
            gearRequestData.update(newPeriod)
        else:
            existingPeriod = {"periodId": periodId}
            gearRequestData.update(existingPeriod)

        savedGearRequest = requests.post(f"https://{DOMAIN}/api/fa/{faId}/gear-requests", json=gearRequestData, headers={
                                         "Authorization": f"Bearer {TOKEN}"}, timeout=5).json()
        periodId = savedGearRequest["rentalPeriod"]["id"]

        previousRentaPeriodId = gearRequest["rentalPeriod"]["id"]
        requests.delete(f"https://{DOMAIN}/api/fa/{faId}/gear-requests/{gearId}/rental-period/{previousRentaPeriodId}",
