...
def deleteLocations(locations):
    for location in locations:
        ...
        requests.delete(f"https://{DOMAIN}/api/signa-location/{location['id']}", headers={
...
def updateLocations(locations):
    for location in locations:
        ...
        requests.patch(f"https://{DOMAIN}/api/signa-location/{location['id']}", json=location, headers={
                       "Authorization": f"Bearer {TOKEN}"}, timeout=5).json()
...
def createLocations(locations):
    for location in locations:
        ...
        json = {"name": location}
        requests.post(f"https://{DOMAIN}/api/signa-location/", json=json,
                      headers={"Authorization": f"Bearer {TOKEN}"}, timeout=5).json()


def getAllLocations():
    return requests.get(f"https://{DOMAIN}/api/signa-location/", headers={"Authorization": f"Bearer {TOKEN}"}, timeout=5).json()
