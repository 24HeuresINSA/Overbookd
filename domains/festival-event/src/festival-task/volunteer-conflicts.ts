...
import { FestivalTask, Conflict, Volunteer, Mobilization } from "./festival-task";
...
export class FestivalTaskTranslator {
  ...
  async translate(
    ...
    const mobilizations = await Promise.all(
      task.mobilizations.map(async (mobilization) => {
        const volunteers = await this.assignConflictsToVolunteers(mobilization, task.id)
  ...
  private async assignConflictsToVolunteers(mobilization: Mobilization<Volunteer>, taskId: FestivalTask["id"]) {
    return Promise.all(
