<template>
  <v-container>
    ...
    <h2>FA</h2>
    <v-text-field label="message de confirmation" v-model="config.fb_confirm_submit"></v-text-field>

    <h2>Dispo</h2>
    <v-text-field label="charisme max" type="number" v-model="config.max_charisma"></v-text-field>
    <v-textarea label="description des dispo" type="number" v-model="config.availabilities_description"></v-textarea>

    <h2>Log</h2>
    <v-switch v-model="config.isInventoryOpen" label="ouvrir l'edition de l'inventaire au hard"></v-switch>
  ...
  data(){
    return {
      config: {
        ...
        availabilities_description: getConfig(this, 'availabilities_description'),
